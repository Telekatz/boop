##############################################
# $Id: 00_CUL.pm 14119 2017-04-27 11:41:18Z rudolfkoenig $
package main;
...
my $clientsBetty     = ":Betty:HMS:CUL_IR:$sccMods:";
...
# Normal devices
  ...
  my @attrList = qw(
    ...
    rfmode:SlowRF,HomeMatic,MAX,WMBus_T,WMBus_S,KOPP_FC,Betty 
    sendpool
    showtime:1,0
  );
  use warnings 'qw';
...
CUL_Parse($$$$@)
{
  ...
  my $dmsgLog = (AttrVal($name,"rfmode","") eq "HomeMatic")
                   ...
                   :$dmsg;
  if($dmsg =~ m/^[AFTKEHRStZriby]([A-F0-9][A-F0-9])+$/) { # RSSI
    my $l = length($dmsg);
...
CUL_Attr(@)
{
  ...
  if($aName eq "rfmode") {
    ...
    if($aVal eq "HomeMatic") {
      ...
      if($hash->{CMDS} =~ m/y/ || IsDummy($hash->{NAME}) || !$hash->{FD}) {
        $hash->{Clients} = $clientsBetty;
        $hash->{MatchList} = \%matchListBetty;
        $hash->{initString} = "X21\nyr"; # krS: start Betty receive Mode
        CUL_WriteInit($hash);
